...
describe('Testing DateHeader Component', () => {
  ...
  it('Given Dateheader When click on the primary header Then it should change the unit', async () => {
    ...
    const { getByTestId } = render(dateHeaderComponent({ unit: 'day', labelFormat: formatlabel, showPeriod }));
    // Arrange
  ...
  it('Given DateHeader component When pass an intervalRenderer prop then it should be called with the right params', () => {
    const intervalRenderer = jest.fn(({ getIntervalProps, intervalContext, data }) => (
...
function dateHeaderComponent({ labelFormat, unit, props, className, style, showPeriod } = {}) {
  return (
  ...
  return (
    <RenderHeadersWrapper timelineState={{ timelineUnit: 'month' }}>
      <TimelineHeaders>
        ...
        <DateHeader unit="primaryHeader" />
        <DateHeader unit={'day'} labelFormat={'MM/DD/YYYY'} headerData={props} intervalRenderer={intervalRenderer} />
        <DateHeader />
      </TimelineHeaders>
    </RenderHeadersWrapper>
  );
